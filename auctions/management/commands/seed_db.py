    ...
    def handle(self, *args, **kwargs):
        ...
        for i, car in enumerate(car_data):
            auction = Auction.objects.create(
                ...
                + timezone.timedelta(days=7 + i * 2),  # Stagger end times
            )
            ...
            Listing.objects.create(
                ...
                start_time=timezone.now(),
                end_time=timezone.now()
                + timezone.timedelta(days=5 + i),  # Stagger end times
            )

        self.stdout.write(
            self.style.SUCCESS("Successfully seeded the database with dynamic data")
        )
